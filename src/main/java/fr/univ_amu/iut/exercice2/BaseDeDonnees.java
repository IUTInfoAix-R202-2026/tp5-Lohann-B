 ...
 *
 * <p>
 * À l'exercice 1, on ouvrait une {@link Connection} « à la main » avec le
 * {@code DriverManager}.
 * En pratique, on travaille plutôt avec une {@link DataSource} : un objet qui
 * sait fournir des
 * connexions. C'est l'abstraction que toutes les couches au-dessus (DAO,
 * services) réclameront, et
 * qu'on pourra injecter (cf. capstone). Les DAO des exercices suivants
 * reçoivent une {@code
 * DataSource} et ne connaissent jamais l'URL JDBC.
 *
 * <p>
 * Cette classe expose aussi {@link #initialiser(DataSource)} qui crée les
 * tables ({@code
 * schema.sql}) et insère les données du fil rouge ({@code seed.sql}). Le
...
public class BaseDeDonnees {
  ...
  /**
   * Crée une {@link DataSource} SQLite pointant vers le fichier donné, avec
   * l'intégrité
  ...
  /**
   * Crée les tables (schema.sql) puis insère les données du fil rouge (seed.sql).
   * Fourni. AAAH
   */
